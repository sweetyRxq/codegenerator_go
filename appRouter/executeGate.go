...
func ProcessBusiness_Invoke(stub shim.ChaincodeStubInterface, tranCode string, requestMsg *systematic.Message, returnMsg *systematic.Message ) {
	...
	case "UpdateResourceCar":
		...
		break
	case "CreateResourceBox":
		log.Info("Request Message Id:" + requestMsg.Id + " CreateResourceBox perform start")
		...
		var resourceBoxJSON string
		var fileArr []string
		//如果涉及到了附件信息
		if hasFile == "true"{
			dataArr, err := systematic.GetDataFromMessage(requestMsg) //获取到的是个data数组
			if err != nil{
				systematic.CombinationErrorObj(err.Error(), "CreateResourceBox", systematic.ERRNMCC0012, returnMsg)
				systematic.ErrorMessage(returnMsg)
			}

			for _, dataItem := range dataArr{
				if dataItem.DataType == "ResourceBox"{
					resourceBoxJSON = dataItem.Content
				}else if dataItem.DataType == "FileInfo"{
					fileArr = append(fileArr, dataItem.Content)
				}
			}
			...
			//如果是单纯的新增业务数据
			tmpResourceBoxJSON, err := systematic.GetOneDataFromMessage(requestMsg)
			if err != nil{
				systematic.CombinationErrorObj(err.Error(), "CreateResourceBox", systematic.ERRNMCC0012, returnMsg)
				systematic.ErrorMessage(returnMsg)
			}

			resourceBoxJSON = tmpResourceBoxJSON
		}

		err, errCode := model.AddResourceBox(stub, resourceBoxJSON, fileArr)
		if err != nil{
			systematic.CombinationErrorObj(err.Error(), "CreateResourceBox", errCode, returnMsg)
			systematic.ErrorMessage(returnMsg)
		}

		log.Info("Return Message Id:" + returnMsg.Id + " CreateResourceBox perform finish")
		break
	case "DelResourceBox":
		log.Info("Request Message Id:" + requestMsg.Id + " DelResourceBox perform start")
		var args = systematic.GetArgsOfDelOrQuery(requestMsg)
		var id string
		...
		err, errCode := model.DelResourceBox(stub, id)
		if err != nil{
			systematic.CombinationErrorObj(err.Error(), "DelResourceBox", errCode, returnMsg)
			systematic.ErrorMessage(returnMsg)
		}
		log.Info("Return Message Id:" + returnMsg.Id + " DelResourceBox perform finish")
		break
	case "UpdateResourceBox":
		log.Info("Request Message Id:" + requestMsg.Id + " UpdateResourceBox perform start")
		resourceBoxJSON, err := systematic.GetOneDataFromMessage(requestMsg)
		if err != nil{
			log.Error(systematic.ERRNMCC0012)
			systematic.CombinationErrorObj(err.Error(), "UpdateResourceBox", systematic.ERRNMCC0012, returnMsg)
			systematic.ErrorMessage(returnMsg)
		}

		err, errCode := model.UpdateResourceBox(stub, resourceBoxJSON)
		if err != nil{
			systematic.CombinationErrorObj(err.Error(), "UpdateResourceBox", errCode, returnMsg)
			systematic.ErrorMessage(returnMsg)
		}
		log.Info("Return Message Id:" + returnMsg.Id + " UpdateResourceBox perform finish")
		break
	case "FileInvoke":
...
func ProcessBusiness_Query(stub shim.ChaincodeStubInterface, tranCode string, requestMsg *systematic.Message, returnMsg *systematic.Message ) {
	...
	case "PaginateResourceCar":
		...
		break;
	case "SelectAllResourceBox":
		log.Info("Request Message Id:" + requestMsg.Id + " SelectAllResourceBox perform start")
		err, errCode, resArr := model.SelectAllResourceBox(stub)
		if err != nil{
			systematic.CombinationErrorObj(err.Error(), "SelectAllResourceBox", errCode, returnMsg)
			systematic.ErrorMessage(returnMsg)
		}else {
			for _, item := range resArr{
				var data systematic.Data
				data.Content = item
				data.DataType = "ResourceBox"

				returnMsg.Data = append(returnMsg.Data, data)
			}
		}
		log.Info("Return Message Id:" + returnMsg.Id + " SelectAllResourceBox perform finish")
		break
	case "SelectResourceBox":
		log.Info("Request Message Id:" + requestMsg.Id + " SelectResourceBox perform start")
		resourceBoxJSON, err := systematic.GetOneDataFromMessage(requestMsg)
		if err != nil{
			systematic.CombinationErrorObj(err.Error(), "SelectResourceBox", systematic.ERRNMCC0012, returnMsg)
			systematic.ErrorMessage(returnMsg)
		}

		var resourceBoxObj *model.ResourceBox
		err = json.Unmarshal([]byte(resourceBoxJSON), &resourceBoxObj)
		if err != nil{
			systematic.CombinationErrorObj(err.Error(), "SelectResourceBox", systematic.ERRNMCC0001, returnMsg)
			systematic.ErrorMessage(returnMsg)
		}else {
			...
			if resourceBoxKey == ""{
				...
				err, errCode, result := model.SelectResourceBox(stub, resourceBoxKey)
				if err != nil{
					systematic.CombinationErrorObj(err.Error(), "SelectResourceBox", errCode, returnMsg)
					systematic.ErrorMessage(returnMsg)
				}
		...
		log.Info("Return Message Id:" + returnMsg.Id + " SelectResourceBox perform finish")
		break
	case "QueryResourceBox":
		log.Info("Request Message Id:" + requestMsg.Id + " QueryResourceBox perform start")
		// 获取QueryString
		var args = systematic.GetArgsOfDelOrQuery(requestMsg)
		var queryString string
		for _, argsItem := range args {
		...
		// 如果queryString为空
		if queryString == "" {
			queryString = "{\"dataType\":\"ResourceBox\"}"
		}
		err, errCode, resArr := model.QueryResourceBox(stub, queryString)
		if err != nil{
			systematic.CombinationErrorObj(err.Error(), "SelectAllResourceBox", errCode, returnMsg)
			systematic.ErrorMessage(returnMsg)
		}else {
		...
		log.Info("Return Message Id:" + returnMsg.Id + " QueryResourceBox perform finish")
		break
	case "PaginateResourceBox":
		log.Info("Request Message Id:" + requestMsg.Id + " PaginateResourceBox perform start")
		systematic.PaginateQuery(stub, requestMsg, returnMsg)
		log.Info("Return Message Id:" + returnMsg.Id + " PaginateResourceBox perform finish")
		break;
	case "FileQuery":
